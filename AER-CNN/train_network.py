...
def get_total_files(path="Preproc/",train_percentage=0.8): 
    ...
    for subdir in subdirs:
        #subdir = ESC-50-master
        #print(path+ '//' +subdir)
        pass
        files = os.listdir(path+'/'+subdir) # files = Preproc/ESC-50-master
        n_files = len(files)
        sum_total += n_files
...
def build_datasets(train_percentage=0.8, path='Preproc'):
    ...
    print("shape of each data file = ",inputShape)
...
if __name__ == '__main__':
    ...
    # Training the model
    model.fit(X_train,Y_train, validation_data=(X_validation,Y_validation),epochs=20,verbose=2)
    # Testing the model
