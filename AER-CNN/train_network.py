...
def get_class_names(path=r"Preproc/"):
    class_names = os.listdir(path)
    return class_names


# function to get the count of total, training and testing files
def get_total_files(path=r"Preproc\\", train_percentage=0.8):
    sum_total = 0
    sum_train = 0
    sum_test = 0
...
def get_sample_dimensions(path=r'Preproc/'):
...
def build_datasets(train_percentage=0.8, path=r'Preproc'):
    ...
    for idx, classname in enumerate(class_names):
        ...
        for idx2, infilename in enumerate(class_files[0:n_load]):
            ...
            sr = 44100
            melgram = melgram[:, :, :,
                      0:inputShape[3]]  # just in case files are differnt sizes: clip to first file size
            if (idx2 < n_train):
