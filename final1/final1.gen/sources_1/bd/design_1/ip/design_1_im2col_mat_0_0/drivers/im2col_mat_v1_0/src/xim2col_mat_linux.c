...
typedef struct {
    ...
    XIm2col_mat_uio_map maps[ MAX_UIO_MAPS ];
} XIm2col_mat_uio_info;

/***************** Variable Definitions **************************************/
static XIm2col_mat_uio_info uio_info;
...
static int uio_info_read_name(XIm2col_mat_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XIm2col_mat_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XIm2col_mat_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XIm2col_mat_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XIm2col_mat_Initialize(XIm2col_mat *InstancePtr, const char* InstanceName) {
	XIm2col_mat_uio_info *InfoPtr = &uio_info;
